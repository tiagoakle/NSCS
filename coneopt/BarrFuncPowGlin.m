function [F,G,H,feas] = BarrFuncPowGlin(x1,x2,x3,K,want)

% -log(x^(2a)*y^(2-2a) - z^2) -max(0,1-2a)*log(x) -max(0,2a-1)*log(y)
...
% Function value:
if want(1) > 0
    F = - sum(log(xi)) - sum(sum(K.powc.a1g.*log(x23)));
end

% Gradient:
if want(2) > 0 || want(3) > 0
    tmp2    = [phi;phi;phi];
    tmp2    = (K.powc.ga2.*tmp2 + K.powc.ga1g).*xm1;
    gt      = -tmp2;
    gt(1,:) = 2*x1 ./ xi;
    G       = gt(:);
end
