%In this test we solve with rhs -s-dr-sigma*mu*g(x)
%and change all variables but s,
%When lambda is small we include dr in the linear term and rhs -s-sigma*mu*g(x)
...
%Calculate lambda
    lambda = (dx./x)'*(dx./x)+dt^2*k/t+dk^2*k/t;
    lambda = sqrt(lambda);
    alpha = 1/(1+lambda);

    %Infeasibility of the full dual step
    dual_inf = min(s+ds);
    fprintf('Iter %i: lambda %3.3d, mu %3.3d, ||d||: %3.3d, ||p||: %3.3d, ||g||: %3.3d, ||o|| %3.3d, s_: %3.3d, mink,s: %3.3d',...
                                                        iter,lambda,mu,ndra,npra,ngra,nthra,dual_inf,min(t,k));
