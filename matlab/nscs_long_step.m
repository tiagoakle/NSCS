...
function [xc,xf,y,s,info] = nscs_long_step(problem,x0f,x0c,pars)
    ...
    for m_iter = 1:pars.max_iter
        ...
        H              = eval_hessian_nt(problem,state.xc,state.s,state.w,state.mu); 
...
function print_f(pars,level,string,varargs)
    if(pars.print>level)
        fprintf(string);
    end
end

%Calculates the product with the matrix defined by the scaling point w
function y = w_times_x(w,x)
