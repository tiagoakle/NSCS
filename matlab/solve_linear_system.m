function [d,slv_aug] = solve_linear_system(H,mu,kappa,tau,problem,pars,r1,r2,r3,r4,r5,factorization)
    
 
    %Populates state.dx,dy,dtau,ds,dkappa
    A  = problem.A;
    b  = problem.b;
    c  = problem.c;
    n  = problem.n;
    ...
    function h_solver = factor()
       %Factorizes
        ...
        function x = solve(L,D,P,y)
            ...
            %[      A ] x(1)   = y(1)
            %[A' -muH ] x(2)     y(2)
            for(iter_round = 1:pars.max_iter_ref_rounds) 
                ri1 = y(1:m) - A*x(m+1:m+n);
                ri2 = y(m+1:m+n) - A'*x(1:m) + mu*H*x(m+1:m+n);
