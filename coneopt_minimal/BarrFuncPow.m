function [F,G,H,feas] = BarrFuncPow(x1,x2,x3,K,want)
...
% Function value:
if want(1) > 0
    F = - sum(log(xi)) - sum(sum(K.powc.a1.*log(x23)));
end

% Gradient:
if want(2) > 0 || want(3) > 0
    tmp2    = [phi;phi;phi];
    tmp2    = (K.powc.ga2.*tmp2 + 1 - K.powc.ga1).*xm1;
    gt      = -tmp2;
    gt(1,:) = 2*x1 ./ xi;
    G       = gt(:);
end
%previously used above^
%tmp2    = (K.powc.ga2.*tmp2 + 1 - K.powc.ga2).*xm1;
% notice: ga2 in the end instead of ga1
% ga1 IS correct. Because using ga2
% gives nu = 2!!! and appears to work better. 
% but it does not satisfy e.g. g'*x = -nu
