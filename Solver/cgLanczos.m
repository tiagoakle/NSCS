function [ x, istop, itn, Anorm, Acond, rnorm, xnorm, D, interrupt_results ] = ...
           cgLanczos( A, b, show, check, itnlim, rtol ,varargin)
...
if ~done                        % k = itn = 1 first time through
  while itn < itnlim
    ...
    %Call the stop interrupt
    if ~isempty(stop_interrupt)&&stop_interrupt(x,interrupt), istop = 7;end
