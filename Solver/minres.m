function [ x, istop, itn, rnorm, Arnorm, Anorm, Acond, ynorm, interrupt_results ] = ...
           minres( A, b, M, shift, show, check, itnlim, rtol, varargin)
%Modified Feb 2013 to accept a stop interrupt handle
%The stop interrupt handle receives x and returns true if the method should stop
% Modified Oct 2012 to accept an interrupt handle
...
%Set up the interrupt cell array
stop_interrupt = [];
interrupt_results = {};
interrupt      = [];
if nargin >= 9
    %interrupt no stop interrupt
    interrupt = varargin{1};
end
if nargin == 10
    stop_interrupt = varargin{2};
end
if nargin <= 8
    interrupt = [];
    stop_interrupt = [];
...
msg = [' beta2 = 0.  If M = I, b and x are eigenvectors '   % -1
       ...
       ' The user defined stoping condition was reached ']; % 10
  ...
  while itn < itnlim
    ...
    if istop==0
     ...
     %if rnorm <= epsr   , istop = 1; end
      ...
      if(~isempty(stop_interrupt)&stop_interrupt(x)),istop = 10; end;
    end
...
%   Call the interrupt and save the results
	if ~isempty(interrupt), interrupt_results = interrupt(x,interrupt_results); end
